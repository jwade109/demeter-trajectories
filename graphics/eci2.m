function eci2(objects, fmts, dots)

if numel(objects) ~= numel(fmts)
    error("bitch");
end
...
if ~isfield(orbit, 'stop')
    orbit.stop = orbit.epoch + orbit.T;
end
...
POS = history(orbit, linspace(orbit.epoch, orbit.stop, 500));
...
% spacecraft point
plot3(orbit.r(1), orbit.r(2), orbit.r(3), dots{i}, 'MarkerSize', 25);
plot3(POS(end,1), POS(end,2), POS(end,3), dots{i}, 'MarkerSize', 25);
