...
earth_parking = elements2orbit((6378+500)*1000,...
    0, 0, 0, 0, 0, mu('earth'));
mars = mars(epoch);
mars_parking = elements2orbit(125*1000,...
    0, 0, 0, 0, 0, mu('mars'));
...
e2 = propagate_to(earth, launch_date + dtof);

[v1, ~, v2, ~] = intercept2(m1.r, e2.r, dtof, mu('sun'));

if norm(v1 - m1.v) < norm(v2 - m1.v)
    t1 = rv2orbit(m1.r, v1, mu('sun'), m1.epoch);
else
    t1 = rv2orbit(m1.r, v2, mu('sun'), m1.epoch);
end
